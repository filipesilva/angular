...
export class PerfTracker implements PerfRecorder {
  ...
  private inSpan: number|null = null;
  ...
  mark(name: string, node?: ts.SourceFile|ts.Declaration, category?: string, detail?: string):
      void {
    const msg = this.makeLogMessage(PerfLogEventType.MARK, name, node, category, detail);
    this.log.push(msg);
  }
  ...
  start(name: string, node?: ts.SourceFile|ts.Declaration, category?: string, detail?: string):
      number {
    if (this.inSpan !== null) {
      this.spanStack.push(this.inSpan);
    }
    this.inSpan = this.nextSpanId++;
    const msg = this.makeLogMessage(PerfLogEventType.SPAN_OPEN, name, node, category, detail);
    this.log.push(msg);
    return this.inSpan;
  }
  ...
  stop(span: number|false): void {
    if (this.inSpan !== span) {
      throw new Error(`Perf Error: stop(${span}) but not currently in that span`);
    }
    this.inSpan = this.spanStack.pop() || null;
    this.log.push({
  ...
  private makeLogMessage(
      ...
      category: string|undefined, detail: string|undefined): PerfLogEvent {
